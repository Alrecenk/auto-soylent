...
var AutoSoylent = AutoSoylent || {};

AutoSoylent.Settings = function () {
...
AutoSoylent.Optimization = function(){
    /**
     ...
     */
    function fitness(ingredientQuantities, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M) {

        var output = AutoSoylent.Vector.create(targetLength),
            totalError = 0, i, t;

        // M*x - 1
        for (t = 0; t < targetLength; t++) {
            // Calculate output
            output[t] = 0;
            for (i = 0; i < ingredientQuantities.length; i++) {
                output[t] += M[i][t] * ingredientQuantities[i];
            }
        ...
        var penalty = 0;
        for (i = 0; i < ingredientQuantities.length; i++) {
            penalty += costs[i] * ingredientQuantities[i];
        }
    ...
    /**
     * Gradient of fitness with respect to x.
     ...
     * @author Alrecenk (Matt McDaniel) of Inductive Bias LLC (www.inductivebias.com) March 2014
     */
    function gradient(x, targetLength, M, lowWeight, highWeight, maxPerMin, w, costs) {
        var ingredientLength, output, t, i;

        ingredientLength = x.length;
        output = AutoSoylent.Vector.create(targetLength);
...
//returns multiple of g you should move in
    function stepSize(x, g, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M) {
        var fv = fitness(x, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
        var newf;
        var newx = [];
        ...
        while (linesearch) {
            ...
            newf = fitness(newx, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M); // Get fitness
        ...
        if (done) {
            ...
            var midf = fitness(midx, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
            ...
            var optf = fitness(optx, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
            //keep whichever new point is best
            //console.log( fv +", " + newf +", " + midf +", " + optf) ;
            //we already know new fitness is less than first fitness
            fv = newf;
...
AutoSoylent.Vector = function(){
     ...
     * @returns {Array}
     */
    function scale(vector, scaleFactor) {
        var scaledVector = [];
        for (var k = 0; k < vector.length; k++) {
            scaledVector[k] = vector[k] * scaleFactor;
        }
        return scaledVector;
    }
    ...
    function createVector(length) {
        ...
        if (arguments.length > 1) {
            ...
            while (i--) arr[length - 1 - i] = createVector.apply(this, args);
        }
...
AutoSoylent.Main = (function(){
    function execute(request){

        // Weight cost regularization (creates sparse recipes for large numbers of ingredient, use 0 for few ingredients)
        var w = 0.07;
        var maxPerMin;    // Ratio of maximum value to taget value for each ingredient
        var lowWeight;
        var highWeight;		// How to weight penalties for going over or under a requirement
        var maxFractionAllowed = 0.9; // recipes are penalized for going above this fraction of the maximum in the nutrient profile
        var macroWeight = 3; // how macro nutrients are weighted compared to micro
        var maxMacroRatio = 1.1; //the maximum amount of macro nutrients allowed relative to minimum
        ...
        var recipes = AutoSoylent.Settings.recipes;

        for (var i = 0; i < recipes.length; i++) {
            request.get(recipes[i] + "/json?nutrientProfile=" + AutoSoylent.Settings.nutrientProfile, function (err, response) {
                if (err) {
                ...
                var result = complete(completions, recipes, recipeIngredients, ingredients, nutrientTargets, lowWeight, maxPerMin, highWeight, w, nutrients, maxFractionAllowed, macroNutrients, macroWeight, maxMacroRatio, require);
                AutoSoylent.TableFormatter.format(result);
            });
        }
    }

    function complete(completions, recipes, recipeIngredients, ingredients, nutrientTargets, lowWeight, maxPerMin,
                      highWeight, w, nutrients, maxFractionAllowed, macroNutrients, macroWeight, maxMacroRatio,
                      enableServingSizeCorrection) {
        var i, j;
        ...
        if (completions >= recipes.length) {
            ...
            var ingredientQuantities = generateRecipe(ingredients, nutrientTargets, lowWeight, maxPerMin, highWeight, w,
                nutrients, maxFractionAllowed, macroNutrients, macroWeight, maxMacroRatio, enableServingSizeCorrection);
        ...
        // fallback when not all recipes have completed
        return {
            "ingredients": ingredients,
            "ingredientQuantities": AutoSoylent.Vector.create(ingredients.length),
            "nutrientTargets": nutrientTargets,
            "nutrients": nutrients
        }
    }
     ...
     * @author Alrecenk (Matt McDaniel) of Inductive Bias LLC (www.inductivebias.com) March 2014
     */
    function generateRecipe(ingredients, nutrientTargets, lowWeight, maxPerMin, highWeight, w, nutrients, maxFractionAllowed, macroNutrients, macroWeight, maxMacroRatio, enableServingSizeCorrection) {
        ...
        for (t = 0; t < targetAmount.length; t++) {
        ...
        // Length of ingredient and target array (also dimensions of m)
        var ingredientLength = ingredients.length;

        var targetLength = targetAmount.length;

        // Matrix mapping ingredient amounts to chemical amounts (values are fraction per serving of target value)
        var M = AutoSoylent.Vector.create(ingredientLength, targetLength);

        // Cost of each ingredient per serving
        var costs = [];
        ...
        for (i = 0; i < ingredients.length; i++) {
            ...
            if (isNaN(ingredients[i].container_size) || ingredients[i].container_size <= 0 || isNaN(ingredients[i].serving) || ingredients[i].serving <= 0) {
                ...
                costs[i] = ingredients[i].item_cost;
                x[i] = ingredients[i].amount;
            }
            else {
                costs[i] = ingredients[i].item_cost * ingredients[i].serving / ingredients[i].container_size;
        ...
        var pricePerDay = 0;
        for (k = 0; k < x.length; k++) {
            pricePerDay += x[k] * costs[k];
        }

        console.log("Starting price per day: $" + pricePerDay.toFixed(2));
        console.log("Calculating Optimal Recipe...");

        var fv = AutoSoylent.Optimization.fitness(x, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
        var g = AutoSoylent.Optimization.gradient(x, targetLength, M, lowWeight, highWeight, maxPerMin, w, costs);
        var iteration = 0;
        var newx = [];
        var done = false;
        var stepsize;
        ...
        while (!done && iteration < 50000) { // Loops until no improvement can be made or max iterations
            ...
            scaledGradient = AutoSoylent.Vector.scale(g, -1);
            stepsize = AutoSoylent.Optimization.stepSize(x, scaledGradient, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
            done = (stepsize == 0);
            if (!done) {
                ...
                x = newx;
                fv = AutoSoylent.Optimization.fitness(x, targetLength, lowWeight, maxPerMin, highWeight, costs, w, M);
                g = AutoSoylent.Optimization.gradient(x, targetLength, M, lowWeight, highWeight, maxPerMin, w, costs);
            }
        }
...
AutoSoylent.TableFormatter = function(){
    function format(result) {
        var ingredients = result["ingredients"];
        var ingredientQuantities = result["ingredientQuantities"];
        var nutrientTargets = result["nutrientTargets"];
        var nutrients = result["nutrients"];
        ...
        for (i = 0; i < ingredients.length; i++) {
            ingredientsTable.push([ingredients[i].name, ingredients[i].amount + " " + ingredients[i].unit,
        ...
        for (var nutrient in nutrientTargets) {
            if (nutrients.indexOf(nutrient) < 0) {
                // Skip over non-nutrient properties
                continue;
            }
...
var Table = require('cli-table'); // does not work in PowerShell

AutoSoylent.Main.execute(request);
