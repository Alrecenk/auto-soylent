...
var nutrientProfile = "5338f539c9d2a002007afe6a" ; //"51e4e6ca7789bc0200000007"// standard nutrient profile
var recipes = ["http://diy.soylent.me/recipes/flaxcocopea"

//"http://diy.soylent.me/recipes/oz-beta-tweaked"

//"diy.soylent.me/recipes/optimized-top-five-remix-simplified"
/*
...
var ingredientLength,
    ...
    w = 0.001,    // Weight cost regularization (creates sparse recipes for large numbers of ingredient, use 0 for few ingredients)
	...
	discardFreeItems = true, //items with no cost cause problems, enable this to precent them from being used
	startOptimizationfromZero = true, //if true all ingredients begin at 0, otherwise they begin at the recipe
	macroWeight = 5 ; // how macro nutrients are weighted compared to micro
	maxMacroRatio = 1.02; //the maximum amount of macro nutrients allowed relative to minimum
	;   
...
var nutrients = [
    ...
    'panthothenic', 'phosphorus', 'potassium', 'protein', 'riboflavin', 'selinium', 'sodium', 'thiamin',
...
function generateRecipe(ingredients, nutrientTargets){
 ...
 targetName = [], x = []; // Number of servings of each ingredient
	...
	for (var i = 0; i < ingredients.length; i++) {
		...
		pricePerDay += x[i] * cost[i];
		if (startOptimizationfromZero) {
			x[i] = 0;
		}
		...
		if (cost[i] <= 0.0000001) {
			...
			if (discardFreeItems) {
				...
				console.log("Warning: " + ingredients[i].name + " does not have a cost. It will still be used. Error possible.");
			}
		}
	...
	while (!done && iteration < 5000000) { // Loops until no improvement can be made or max iterations
		iteration++;
...
function parabolicInterpolation( x1, fx1, x2, fx2, x3, fx3){
	...
	var newf ;
	var newx = [] ;
	var stepsize = 2000; // Start with big step
 	var linesearch = true;
	var done = false ;
	while (linesearch) {
		newx = [] ;
