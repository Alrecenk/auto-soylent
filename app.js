/**
 * Nonlinear Auto-Soylent Solver v0.3
 *
...
//var recipeUrl = "http://diy.soylent.me/recipes/simplecheapvegannosoy-soylent-low-carb" ;
//var recipeUrl = "http://diy.soylent.me/recipes/cheap-chocolate-oat-complete" ;
//var recipeUrl = "http://diy.soylent.me/recipes/oats-whey-n-maltodextrin" ;
//var nutrientProfile = "51e4e6ca7789bc0200000007"// standard nutrient profile
var recipes = ["http://diy.soylent.me/recipes/oats-whey-n-maltodextrin"] ;
var nutrientProfile = "5330c6e624b6250200777ec5" ; // nutrient profile for oats-whey-n-maltodextrin
...
var ingredientLength,
    ...
    w = 0.01,    // Weight cost regularization (creates sparse recipes for large numbers of ingredient, use 0 for few ingredients)
...
function generateRecipe(ingredients, nutrientTargets) {
    ...
    for (var t = 0; t < targetAmount.length; t++) {
        ...
        if (nutrientTargets[targetName[t] + "_max"] > targetAmount[t]) {
            ...
            maxPerMin[t] = 0.95*maxvalue / targetAmount[t]; // Record it
        }
        else {
    ...
    for (var i = 0; i < ingredients.length; i++) {
        for (var t = 0; t < targetAmount.length; t++) {
			...
			if (isNaN(ingredients[i][targetName[t]])) {
				...
				if(isNaN(M[i][t])){
					console.log( "Error: Invalid value in ingredient amount:" +ingredients[i][targetName[t]])
				}
			}
		...
		if(isNaN ( ingredients[i].container_size) ||  ingredients[i].container_size <=0){
		...
		if(isNaN(cost[i])){
			console.log( "Error: Invalid value in cost:" +ingredients[i].item_cost + " * " + ingredients[i].serving +" /"+ ingredients[i].container_size)
		}
